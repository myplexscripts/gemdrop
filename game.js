(() => {
  ...
  class GameScene extends Phaser.Scene {
    ...
    processMerges() {
      ...
      for(const pair of queue){
        ...
        if(next>=tiers.length){
          ...
          this.floatText(x,y-8,'MASTER CUT +
        ...
        this.floatText(x,y-8,label,this.mergeChain>=2?'#ffd86f':'#ffe7c5',this.mergeChain>=2?24:18);
    ...
    floatText(x,y,text,color,size) {
      ...
      const isBig=isChain||isMaster;
      const baseSize=Math.max(16,size||18);
      const popSize=isMaster?Math.max(baseSize,26):isChain?Math.max(baseSize,24):baseSize;
      ...
      const label=this.add.text(x,y,text,{
        ...
        .setAlpha(0)
        .setScale(.58)
        .setAngle(Phaser.Math.FloatBetween(-2.2,2.2));
      ...
      this.tweens.add({
        targets:label,
        alpha:1,
        scaleX:isBig?1.12:1.06,
        scaleY:isBig?0.90:0.94,
        y:y-10,
        angle:0,
        duration:125,
        ease:'Back.Out',
