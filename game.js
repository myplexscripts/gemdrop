(() => {
  ...
  class GameScene extends Phaser.Scene {
    ...
    create() {
      ...
      this.gemMaskShape=this.make.graphics({add:false});
    ...
    useCascade() {
      ...
      for(let i=0;i<this.gems.length;i++){
        ...
        for(let j=i+1;j<this.gems.length;j++){
          const b=this.gems[j];
          if(!b?.active||b.merging||a.tier!==b.tier) continue;
      ...
      bestPair[0].merging=true;
      bestPair[1].merging=true;
      this.pendingMerges.push(bestPair);
...
+fmt(this.best);
    ...
    startRun() {
      ...
      statusHud.classList.remove('show');
    ...
    showCutToast(text) {
      statusHud.textContent=text;
      statusHud.classList.add('show');
      window.clearTimeout(this.statusHudTimer);
      this.statusHudTimer=window.setTimeout(()=>statusHud.classList.remove('show'),1150);
    }
